	...
	for {
		...
		case node := <-nodesCh:
			if err := d.checkDial(node); err != nil {
				logs.Trace("Discarding dial candidate", "id", node.ID(), "ip", node.IP(), "reason", err)
			} else {
		...
		case node := <-d.addStaticCh:
			...
			logs.Trace("Adding static node", "id", id, "ip", node.IP(), "added", !exists)
			if exists {
				continue loop
			}
		...
		case node := <-d.remStaticCh:
			id := node.ID()
			task := d.static[id]
			logs.Trace("Removing static node", "id", id, "ok", task != nil)
			if task != nil {
				delete(d.static, id)
...
func (d *dialScheduler) logStats() {
	...
	if d.dialPeers < dialStatsPeerLimit && d.dialPeers < d.maxDialPeers {
		logs.Info("Looking for peers", "peercount", len(d.peers), "tried", d.doneSinceLastLog, "static", len(d.static))
	}
...
func (d *dialScheduler) startDial(task *dialTask) {
	logs.Trace("Starting p2p dial", "id", task.dest.ID(), "ip", task.dest.IP(), "flag", task.flags)
...
func (t *dialTask) resolve(d *dialScheduler) bool {
	...
	if resolved == nil {
		...
		logs.Debug("Resolving node failed", "id", t.dest.ID(), "newdelay", t.resolveDelay)
		return false
	}
	// The node was found.
	t.resolveDelay = initialResolveDelay
	t.dest = resolved
	logs.Debug("Resolved node", "id", t.dest.ID(), "addr", &net.TCPAddr{IP: t.dest.IP(), Port: t.dest.TCP()})
	return true
}
...
func (t *dialTask) dial(d *dialScheduler, dest *enode.Node) error {
	...
	if err != nil {
		logs.Trace("Dial error", "id", t.dest.ID(), "addr", nodeAddr(t.dest), "conn", t.flags, "err", cleanupDialErr(err))
