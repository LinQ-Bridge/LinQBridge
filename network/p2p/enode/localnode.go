...
func NewLocalNode(key *ecdsa.PrivateKey) *LocalNode {
	ln := &LocalNode{
		...
		//db:      db,
		key:     key,
		entries: make(map[string]enr2.Entry),
		endpoint4: lnEndpoint{
...
// the last change.
func (ln *LocalNode) Set(e enr2.Entry) {
	ln.mu.Lock()
	defer ln.mu.Unlock()
...
func (ln *LocalNode) Delete(e enr2.Entry) {
	ln.mu.Lock()
	defer ln.mu.Unlock()
...
func (ln *LocalNode) updateEndpoints() {
	...
	if ip4 != nil && !ip4.IsUnspecified() {
		ln.set(enr2.IPv4(ip4))
	} else {
		ln.delete(enr2.IPv4{})
	}
	if ip6 != nil && !ip6.IsUnspecified() {
		ln.set(enr2.IPv6(ip6))
	} else {
		ln.delete(enr2.IPv6{})
	}
	if udp4 != 0 {
		ln.set(enr2.UDP(udp4))
	} else {
		ln.delete(enr2.UDP(0))
	}
	if udp6 != 0 && udp6 != udp4 {
		ln.set(enr2.UDP6(udp6))
	} else {
		ln.delete(enr2.UDP6(0))
	}
}
