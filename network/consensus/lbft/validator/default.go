...
type defaultSet struct {
	validators lbft.Validators
	policy     lbft.ProposerPolicy

	proposer    lbft.Validator
	validatorMu sync.RWMutex
	selector    lbft.ProposalSelector
}

func newDefaultSet(addrs []common.Address, policy lbft.ProposerPolicy) *defaultSet {
	valSet := &defaultSet{}

	valSet.policy = policy
	// init validators
	valSet.validators = make([]lbft.Validator, len(addrs))
...
func (valSet *defaultSet) List() []lbft.Validator {
	valSet.validatorMu.RLock()
	defer valSet.validatorMu.RUnlock()
	return valSet.validators
}

func (valSet *defaultSet) GetByIndex(i uint64) lbft.Validator {
	valSet.validatorMu.RLock()
	defer valSet.validatorMu.RUnlock()
...
func (valSet *defaultSet) GetByAddress(addr common.Address) (int, lbft.Validator) {
...
func (valSet *defaultSet) GetProposer() lbft.Validator {
...
func calcSeed(valSet lbft.ValidatorSet, proposer common.Address, round uint64) uint64 {
	offset := 0
...
func roundRobinProposer(valSet lbft.ValidatorSet, proposer common.Address, round uint64) lbft.Validator {
	if valSet.Size() == 0 {
		return nil
	}
	seed := uint64(0)
...
func stickyProposer(valSet lbft.ValidatorSet, proposer common.Address, round uint64) lbft.Validator {
	if valSet.Size() == 0 {
		return nil
	}
	seed := uint64(0)
...
func (valSet *defaultSet) Copy() lbft.ValidatorSet {
	valSet.validatorMu.RLock()
	defer valSet.validatorMu.RUnlock()
...
func (valSet *defaultSet) Policy() lbft.ProposerPolicy { return valSet.policy }
