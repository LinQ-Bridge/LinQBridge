...
func newRoundState(view *consensus.View, validatorSet lbft.ValidatorSet, lockedHash common.Hash, preprepare *consensus.Preprepare, pendingRequest *consensus.Request) *roundState {
	return &roundState{
		round:          view.Round,
		...
		NotReadies:     newMessageSet(validatorSet),
...
func (s *roundState) SetPreprepare(preprepare *consensus.Preprepare) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *roundState) Proposal() consensus.Proposal {
	s.mu.RLock()
	defer s.mu.RUnlock()
